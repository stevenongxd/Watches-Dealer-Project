...
public class CartController {
	...
	public static ObservableList<Cart> getAllCart() {
		...
		String query = String.format("SELECT * FROM cart WHERE UserID = '%d'", UserController.getCurrentUser().getUserID());
	...
	public static void addCart(Integer watchID, Integer quantity) {
		String query = String.format("INSERT INTO `cart`(`UserID`, `WatchID`, `Quantity`) VALUES ('%d','%d','%d')",
				UserController.getCurrentUser().getUserID(), watchID, quantity);
		connect.executeUpdate(query);
	}

	public static void deleteCart() {
		String query = String.format("DELETE FROM `cart` WHERE UserID = '%d'", UserController.getCurrentUser().getUserID());
		connect.executeUpdate(query);
	}
}
